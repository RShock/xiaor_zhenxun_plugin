...
from .utils import (
    ...
    init_game_name_cache,
    get_chinese_game_name,
)
...
game_name_cache_path = store.get_data_file("nonebot_plugin_steam_info", "game_name_cache.json")

init_game_name_cache(game_name_cache_path)
...
async def broadcast_steam_info(
    ...
    for entry in events:
        ...
        old_game_name = old_player.get("gameextrainfo") if old_player else ""

        gameid = player.get("gameid")
        old_gameid = old_player.get("gameid") if old_player else None

        if gameid:
            chinese_name = await get_chinese_game_name(str(gameid), config.proxy)
            if chinese_name:
                game_name = chinese_name
        if old_gameid:
            chinese_name = await get_chinese_game_name(str(old_gameid), config.proxy)
            if chinese_name:
                old_game_name = chinese_name

        if entry["type"] == "start":
            msg.append(f"{player['personaname']} 开始玩 {game_name} 了")
        elif entry["type"] in ["stop", "change"]:
            time_start = old_player["game_start_time"]
            time_stop = entry.get("stop_time", time.time())
            ...
            if entry["type"] == "change":
                msg.append(
                    f"{player['personaname']} 玩了 {time_str} {old_game_name} 后，开始玩 {game_name} 了"
                )
            else:
                msg.append(
                    f"{player['personaname']} 玩了 {time_str} {old_game_name} 后不玩了"
                )
    ...
    if config.steam_broadcast_type == "all":
        ...
        for entry in events:
            if entry["type"] == "start":
                entry_gameid = entry["player"].get("gameid")
                entry_game_name = entry["player"]["gameextrainfo"]
                if entry_gameid:
                    chinese_name = await get_chinese_game_name(str(entry_gameid), config.proxy)
                    if chinese_name:
                        entry_game_name = chinese_name
...
async def update_steam_info(init_mode: bool = False):
    steam_ids = bind_data.get_all_steam_id()
    logger.info(f"[Steam 播报] update_steam_info 开始, 共 {len(steam_ids)} 个已绑定 Steam ID")
    ...
    if steam_info["response"]["players"] != []:
        events = steam_info_data.update_by_players(steam_info["response"]["players"], init_mode=init_mode)
        steam_info_data.save()
        logger.info(f"[Steam 播报] update_steam_info 成功, 获取到 {len(steam_info['response']['players'])} 个玩家, 产生 {len(events)} 个事件")
        asyncio.ensure_future(
            prefetch_avatars(
                steam_info["response"]["players"], avatar_path, config.proxy
            )
        )
    else:
        logger.info(f"[Steam 播报] update_steam_info 未获取到玩家数据")

    return bind_data, events
...
async def fetch_and_broadcast_steam_info():
    bind_data, events = await update_steam_info()

    for parent_id in bind_data.content.keys():
        parent_steam_ids = set(bind_data.get_all(parent_id))
        parent_events = [e for e in events if e["player"]["steamid"] in parent_steam_ids]
        new_players = steam_info_data.get_players(bind_data.get_all(parent_id))

        await broadcast_steam_info(parent_id, parent_events, new_players)


@scheduler.scheduled_job("interval", hours=24, id="refresh_steam_avatars")
async def refresh_steam_avatars():
    steam_ids = bind_data.get_all_steam_id()
    if not steam_ids:
        return
    logger.info(f"[Steam 头像] 定时刷新开始, {len(steam_ids)} 个玩家")
    steam_info = await get_steam_users_info(
        steam_ids, config.steam_api_key, config.proxy
    )
    players = steam_info.get("response", {}).get("players", [])
    if players:
        await prefetch_avatars(players, avatar_path, config.proxy)


async def _startup_sync():
    """bot 启动/重连时仅同步状态，不推进 pending_stop 计数、不播报"""
    await update_steam_info(init_mode=True)


if not config.steam_disable_broadcast_on_startup:
    nonebot.get_driver().on_bot_connect(_startup_sync)
else:
...
async def check_handle(
    ...
    logger.info(
        f"[Steam 查看] check_handle 开始, parent_id={parent_id}, "
        f"已绑定 Steam ID 数: {len(steam_ids)}"
    )
    ...
    if steam_info["response"]["players"] == []:
        logger.info(f"[Steam 查看] API 返回空玩家列表, 向用户反馈失败")
        await check.finish("连接 Steam API 失败，请重试")

    logger.info(
        f"[Steam 查看] API 调用成功, parent_id={parent_id}, "
        f"获取到 {len(steam_info['response']['players'])} 个玩家, 开始生成图片"
    )
