...
from .models import PlayerData, PlayerSummaries
...
async def get_steam_users_info(
    ...
    logger.info(
        f"[Steam API] 开始请求 {len(steam_ids)} 个玩家信息"
        f", API Key 数量: {len(steam_api_key)}"
        f", 代理: {proxy or '无'}"
    )
    ...
    if len(steam_ids) > 100:
        ...
        logger.info(
            f"[Steam API] 分批请求完成, 共获取 {len(result['response']['players'])} 个玩家"
        )
        return result
            ...
            try:
                async with httpx.AsyncClient(proxy=proxy) as client:
                    ...
                    if response.status_code == 200:
                        data = response.json()
                        player_count = len(data.get("response", {}).get("players", []))
                        logger.info(
                            f"[Steam API] 请求成功 (key={masked_key}, "
                            f"attempt={attempt + 1}/{MAX_RETRIES}, "
                            f"返回 {player_count} 个玩家)"
                        )
                        return data
                    else:
                        logger.warning(
                            f"[Steam API] key={masked_key} HTTP {response.status_code}"
                            f" (attempt {attempt + 1}/{MAX_RETRIES})"
                        )
            except httpx.RequestError as exc:
                logger.warning(
                    f"[Steam API] key={masked_key} 请求异常 (attempt {attempt + 1}/{MAX_RETRIES}): {exc}"
                )
            if attempt < MAX_RETRIES - 1:
                delay = RETRY_BASE_DELAY**attempt
                await asyncio.sleep(delay)

    logger.error("[Steam API] 所有 API Key 均已失败, 返回空结果")
    return {"response": {"players": []}}


async def _fetch(
    url: str, default: bytes, cache_file: Path | None = None, proxy: str = None
) -> bytes:
